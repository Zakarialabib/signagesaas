<nav class="mb-8 overflow-x-auto hide-scrollbar">
    <div class="inline-flex p-1 space-x-1 bg-white/60 dark:bg-gray-800/60 shadow-md shadow-indigo-100/20 dark:shadow-black/30 backdrop-blur-md rounded-xl border border-white/20 dark:border-gray-700/30">
        @foreach($navItems as $item)
            <a href="{{ route($item['route']) }}" 
               class="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 
                ...
                aria-current="{{ $active === $item['name'] ? 'page' : 'false' }}">
