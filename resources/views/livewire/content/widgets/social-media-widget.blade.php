                    ...
                    <div class="flex items-center mb-2">
                        ...
                        <div class="ml-3">
                            <p class="text-sm font-medium text-gray-900 dark:text-white">
                                {{ $feed['author'] }}
                                <span class="text-gray-500 dark:text-gray-400 font-normal">{{ $feed['handle'] }}</span>
                            </p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $feed['time'] }}</p>
                        </div>
                    </div>
                    <p class="text-gray-800 dark:text-gray-200">{{ $feed['content'] }}</p>
                    <div class="mt-2 flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400">
                        @if (isset($feed['engagement']['likes']))
                            <span>{{ $feed['engagement']['likes'] }} likes</span>
                        @endif
                        @if (isset($feed['engagement']['retweets']))
                            <span>{{ $feed['engagement']['retweets'] }} retweets</span>
                        @endif
                        @if (isset($feed['engagement']['comments']))
                            <span>{{ $feed['engagement']['comments'] }} comments</span>
                        @endif
                    </div>
                </div>
            @endforeach
        ...
        <div class="space-y-4">
            <div>
                ...
                <div class="mt-2 space-y-2">
                    @foreach (['twitter', 'instagram', 'linkedin'] as $platform)
                        <label class="inline-flex items-center">
                            <input type="checkbox" wire:model="platforms" value="{{ $platform }}"
                            ...
                            <span
                                class="ml-2 text-sm text-gray-700 dark:text-gray-300 capitalize">{{ $platform }}</span>
                        </label>
                    @endforeach
                </div>
            </div>
            ...
            <div>
                <label for="maxItems" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Max Items per
                    Platform</label>
